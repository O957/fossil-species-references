...
from reference_resolver import ReferenceResolver, ReferenceResult, format_reference_citation
...
def enhanced_query_pbdb(
    ...
    try:
        ...
        if resolve_missing and result.get("attribution_mismatch"):
            resolver = ReferenceResolver(use_cache=True, bhl_api_key=bhl_api_key)
            ...
            if original_ref:
                result["original_reference"] = {
                    ...
                    "formatted_citation": format_reference_citation(original_ref)
                }
            else:

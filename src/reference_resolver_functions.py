...
def get_cached_reference(key: str) -> Optional[dict]:
    """
...
def extract_title_from_pbdb_reference(pbdb_reference: str) -> str:
    ...
    year_pattern = r"(\d{4})\.\s*(.+?)(?:\.\s*[A-Z][^.]*|\s+\d+:|\s+\d+\(\d+\)|$)"
...
def search_crossref_by_title(title: str, author_name: str = "", year: str = "") -> Optional[dict]:
    """
    ...
    try:
        ...
        if data.get("message", {}).get("items"):
            for item in data["message"]["items"]:
                ...
                common = {"the", "a", "an", "of", "in", "on", "and", "or", "for", "from", "to", "by", "with"}
                search_words = {w for w in search_words if w not in common and len(w) > 2}
                item_words = {w for w in item_words if w not in common and len(w) > 2}

                # check if significant overlap exists
                if search_words and item_words:
                    overlap = len(search_words & item_words)
                    if overlap >= min(3, len(search_words) * 0.5):  # at least 3 words or 50% match
                        # additional author validation if provided
                        if author_name:
                            authors = item.get("author", [])
                            author_clean = re.sub(r"[()0-9]", "", author_name).strip().lower()
                            if not any(author_clean in auth.get("family", "").lower() for auth in authors):
                                continue
                        ...
                        "year": str(item.get("published-print", {}).get("date-parts", [[year or "Unknown"]])[0][0]),
...
def search_crossref(author_name: str, year: str, taxon_name: str = "") -> Optional[dict]:
    """
...
def search_bhl(author_name: str, year: str, api_key: str = None) -> Optional[dict]:
    """
...
def search_worms(taxon_name: str) -> Optional[dict]:
    """
...
def is_marine_taxon(taxon_name: str) -> bool:
    ...
    """
    marine_indicators = [
        "shark", "ray", "fish", "coral", "squalicorax",
        "enchodus", "mosasaur", "plesiosaur", "ichthyosaur"
    ]
    return any(indicator in taxon_name.lower() for indicator in marine_indicators)
...
def validate_reference_match(pbdb_reference: str, external_reference: dict, attribution_mismatch: bool) -> bool:
    """
    ...
    if ext_title and len(ext_title.strip()) > 10:  # ignore very short titles
        ...
        # remove common words
        common_words = {"the", "a", "an", "of", "in", "on", "and", "or", "for", "with", "by"}
        ext_title_words -= common_words
        pbdb_words -= common_words

        # check for significant word overlap (at least 40% of title words)
        if ext_title_words and len(ext_title_words & pbdb_words) / len(ext_title_words) >= 0.4:
            return True
